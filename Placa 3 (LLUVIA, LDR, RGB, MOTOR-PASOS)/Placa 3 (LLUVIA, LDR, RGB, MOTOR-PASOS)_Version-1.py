...
while True:
    ...
    if estado_actual_agua != estado_anterior_agua:
        if estado_actual_agua == 1:
            ...
            cerrar_techo()
            client.publish(MQTT_TOPIC_LLUVIA, b"Agua detectada. Techo cerrado.")
        else:
            print("✅ No hay agua. Abriendo techo.")
            abrir_techo()
            client.publish(MQTT_TOPIC_LLUVIA, b"No hay agua. Techo abierto.")
    ...
    if estado_actual_luz != estado_anterior_luz:
        if estado_actual_luz == 1:
            ...
            encender_rgb_calido()
            client.publish(MQTT_TOPIC_LUZ, b"Noche detectada. Luz encendida.")
        else:
            print("☀️ Día detectado. Apagando luces.")
            apagar_rgb()
            client.publish(MQTT_TOPIC_LUZ, b"Día detectado. Luz apagada.")
